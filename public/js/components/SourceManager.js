...
class SourceManager {
  ...
  isStreamNetProviderUrl(url) {
    ...
    try {
      ...
      return /(?:^|[\/@.])xui\.(?:streamnet|stremnet)\.live(?::\d+)?(?:\/|$)/i.test(
        raw,
      );
    }
  }
  ...
  showWarningModal({
    ...
    const proceedLabel = proceedText || this.tr("common.proceed");
    const cancelLabel = cancelText || this.tr("common.cancel");
    return new Promise((resolve) => {
      ...
      modalFooter.innerHTML = `
                <button class="btn btn-secondary" id="warning-cancel">${cancelLabel}</button>
                <button class="btn btn-primary" id="warning-proceed" style="background: var(--color-warning, #f59e0b); border-color: var(--color-warning, #f59e0b);">${proceedLabel}</button>
            `;
  ...
  async loadSources() {
    try {
      ...
      this.renderSourceList(
        this.xtreamList,
        sources.filter((s) => s.type === "xtream"),
        "xtream",
      );
      this.renderSourceList(
        this.m3uList,
        sources.filter((s) => s.type === "m3u"),
        "m3u",
      );
      this.renderSourceList(
        this.epgList,
        sources.filter((s) => s.type === "epg"),
        "epg",
      );
    } catch (err) {
  ...
  renderSourceList(container, sources, type) {
    if (sources.length === 0) {
      container.innerHTML = `<p class="hint">${this.tr("sources.noneConfigured", { type: type.toUpperCase() })}</p>`;
      return;
    }
    ...
    container.innerHTML = sources
      ...
      <div class="source-item ${source.enabled ? "" : "disabled"}" data-id="${source.id}">
        ...
        <div class="source-info">
          <div class="source-name">${this.getDisplaySourceName(source)}</div>
    ...
    container.querySelectorAll(".source-item").forEach((item) => {
      ...
      item
        ...
        .addEventListener("click", () => this.refreshSource(id, type));
      item
  ...
  showAddModal(type) {
    ...
    const titles = {
      xtream: this.tr("sources.addXtreamConnection"),
      m3u: this.tr("sources.addM3uPlaylist"),
      epg: this.tr("sources.addEpgSource"),
    };
    ...
    footer.innerHTML = `
            <button class="btn btn-secondary" id="modal-cancel">${this.tr("common.cancel")}</button>
            <button class="btn btn-primary" id="modal-save">${this.tr("sources.addSource")}</button>
    `;
  ...
  async showEditModal(id, type) {
    try {
      ...
      title.textContent = this.tr("sources.editSource", {
      ...
      footer.innerHTML = `
        <button class="btn btn-secondary" id="modal-cancel">${this.tr("common.cancel")}</button>
        <button class="btn btn-primary" id="modal-save">${this.tr("common.saveChanges")}</button>
      `;
  ...
  getSourceForm(type, source = {}) {
    const nameField = `
      <div class="form-group">
                <label for="source-name">${this.tr("common.name")}</label>
                <input type="text" id="source-name" class="form-input" placeholder="${this.tr("sources.mySource")}" value="${this.getDisplaySourceName(source)}">
      </div>
    `;

    const urlField = `
      <div class="form-group">
                <label for="source-url">${type === "xtream" ? this.tr("sources.serverUrl") : this.tr("common.url")}</label>
    ...
    if (type === "xtream") {
      return `
        ...
        ${urlField}
        <div class="form-group">
                    <label for="source-username">${this.tr("users.username")}</label>
          <input type="text" id="source-username" class="form-input" value="${source.username || ""}">
        </div>
        <div class="form-group">
                    <label for="source-password">${this.tr("users.password")}</label>
  ...
  async saveNewSource(type) {
    ...
    const username =
      document.getElementById("source-username")?.value.trim() || null;
    const password =
      document.getElementById("source-password")?.value.trim() || null;
    ...
    try {
        ...
        try {
          ...
          if (estimate.needsWarning) {
            const proceed = await this.showWarningModal({
              title: this.tr("sources.largePlaylistWarningTitle"),
              message: this.tr("sources.largePlaylistWarningMessage", {
                count: estimate.count.toLocaleString(),
              }),
              details: this.tr("sources.largePlaylistWarningDetails"),
              proceedText: this.tr("common.proceedAnyway"),
              cancelText: this.tr("common.cancel"),
            });
            if (!proceed) {
          ...
          console.warn(
            "[SourceManager] Could not estimate M3U size:",
            err.message,
          );
      ...
      alert(this.tr("sources.errorAdding", { message: err.message }));
    }
  }
  ...
  async updateSource(id, type) {
    ...
    try {
      ...
      alert(this.tr("sources.errorUpdating", { message: err.message }));
    }
  }
  ...
  async deleteSource(id) {
    if (!confirm(this.tr("sources.confirmDelete"))) return;
    ...
    try {
      ...
      alert(this.tr("sources.errorDeleting", { message: err.message }));
    }
  }
  ...
  async toggleSource(id) {
    try {
      ...
      alert(this.tr("sources.errorToggling", { message: err.message }));
    }
  }
  ...
  async testSource(id) {
    try {
      ...
      if (result.success) {
        alert(this.tr("sources.connectionSuccessful"));
      } else {
        alert(
          this.tr("sources.connectionFailed", {
            message: result.error || result.message,
          }),
        );
      }
    } catch (err) {
      alert(this.tr("sources.connectionFailed", { message: err.message }));
    }
  }
  ...
  async refreshSource(id, type) {
    try {
      const btn = document.querySelector(
        `.source-item[data-id="${id}"] [data-action="refresh"]`,
      );
      if (btn) {
        btn.disabled = true;
      ...
      if (retries >= maxRetries) {
        throw new Error(this.tr("sources.syncTimedOut"));
      }
      ...
      if (type === "epg") {
        ...
        alert(this.tr("sources.epgSynced"));
      } else if (type === "xtream") {
        // Re-fetch xtream data by reloading channels
        if (window.app?.channelList) {
          await window.app.channelList.loadChannels();
        }
        alert(this.tr("sources.xtreamSynced"));
      } else if (type === "m3u") {
        // Re-fetch M3U data by reloading channels
        if (window.app?.channelList) {
          await window.app.channelList.loadChannels();
        }
        alert(this.tr("sources.m3uSynced"));
      }
      ...
      alert(this.tr("sources.refreshFailed", { message: err.message }));
    }
  }
  ...
  initContentBrowser() {
    ...
    // Source selection
    this.contentSourceSelect?.addEventListener("change", () =>
      this.reloadContentTree(),
    );

    // Show All / Hide All buttons
    document
      .getElementById("content-show-all")
      ?.addEventListener("click", () => this.setAllVisibility(true));
    document
      .getElementById("content-hide-all")
      ?.addEventListener("click", () => this.setAllVisibility(false));

    // Save Changes button
    document
      .getElementById("content-save")
      ?.addEventListener("click", () => this.saveContentChanges());

    // Search input
    const searchInput = document.getElementById("content-search");
    const searchClear =
      searchInput?.parentElement?.querySelector(".search-clear");
      ...
      const typeLabel =
        this.contentType === "movies"
          ...
          : this.contentType === "series"
            ? "series categories"
            : "groups and channels";
      this.contentTree.innerHTML = `<p class="hint">${this.tr("content.selectSourceToView", { typeLabel })}</p>`;
      return;
    }
  ...
  async loadContentSources() {
    try {
      ...
      select.innerHTML = `<option value="">${this.tr("content.selectSource")}</option>`;

      sources
        .filter((s) => s.type === "xtream" || s.type === "m3u")
        .forEach((source) => {
          select.innerHTML += `<option value="${source.id}">${this.getDisplaySourceName(source)} (${source.type})</option>`;
        });
    } catch (err) {
  ...
  async loadContentTree(sourceId) {
    this.contentTree.innerHTML = `<p class="hint">${this.tr("common.loading")}</p>`;
    ...
    try {
      ...
      if (source.type === "xtream" || source.type === "m3u") {
        ...
        const streams = await API.proxy.xtream.liveStreams(sourceId, null, {
          includeHidden: true,
        });
      ...
      this.hiddenSet = new Set(
        hiddenItems.map((h) => `${h.item_type}:${h.item_id}`),
      );
      ...
      this.contentTree.innerHTML = `<p class="hint" style="color: var(--color-error);">${this.tr("content.errorLoading")}</p>`;
    }
  }
  ...
  renderTree() {
    ...
    if (!groups.length) {
      const msg = this.searchQuery
        ? this.tr("content.noMatches")
        : this.tr("content.noneFound");
  ...
  getGroupHtml(group) {
    ...
    const hasVisibleChild = group.items.some(
      (item) => !this.hiddenSet.has(`${item.type}:${item.id}`),
    );
    ...
    if (isExpanded) {
      itemsHtml = `<div class="content-channels">
                ${group.items
                  .map((item) => {
                    const itemHidden = this.hiddenSet.has(
                      `${item.type}:${item.id}`,
                    );
                    return `
  ...
  toggleGroupExpand(groupId) {
    ...
    const groupEl = this.contentTree.querySelector(
      `.content-group[data-group-id="${CSS.escape(groupId)}"]`,
    );
    if (groupEl) {
      ...
      if (group) {
        ...
        const newEl = this.contentTree.querySelector(
          `.content-group[data-group-id="${CSS.escape(groupId)}"]`,
        );
  ...
  async loadMovieCategoriesTree(sourceId) {
    this.contentTree.innerHTML =
      '<p class="hint">Loading movie categories...</p>';
    ...
    try {
      ...
      if (source.type !== "xtream") {
        this.contentTree.innerHTML =
          '<p class="hint">Movie categories are only available for Xtream sources</p>';
        return;
      }
      ...
      if (!categories || categories.length === 0) {
        this.contentTree.innerHTML =
          '<p class="hint">No movie categories found</p>';
        return;
      }
      ...
      this.contentTree.innerHTML =
        '<p class="hint" style="color: var(--color-error);">Error loading movie categories</p>';
    }
  }
  ...
  async loadSeriesCategoriesTree(sourceId) {
    this.contentTree.innerHTML =
      '<p class="hint">Loading series categories...</p>';
    ...
    try {
      ...
      if (source.type !== "xtream") {
        this.contentTree.innerHTML =
          '<p class="hint">Series categories are only available for Xtream sources</p>';
        return;
      }
      ...
      if (!categories || categories.length === 0) {
        this.contentTree.innerHTML =
          '<p class="hint">No series categories found</p>';
        return;
      }
      ...
      this.contentTree.innerHTML =
        '<p class="hint" style="color: var(--color-error);">Error loading series categories</p>';
    }
  }
  ...
  toggleGroupChildren(groupCb) {
    ...
    const groupEl = this.contentTree.querySelector(
      `.content-group[data-group-id="${CSS.escape(group.id)}"]`,
    );
    if (groupEl) {
  ...
  async setAllVisibility(visible) {
    ...
    const hideAllBtn = document.querySelector(
      ".content-actions button:nth-child(2)",
    );
    ...
    if (saveBtn) {
      ...
      saveBtn.textContent = visible
        ? this.tr("content.showingAll")
        : this.tr("content.hidingAll");
    }
    ...
    try {
      ...
      if (saveBtn) {
        saveBtn.textContent = this.tr("common.done");
        setTimeout(() => {
          saveBtn.textContent = this.tr("common.saveChanges");
          saveBtn.disabled = false;
        }, 1500);
      }
    } catch (err) {
      console.error("Error setting all visibility:", err);
      alert(this.tr("common.failedWithMessage", { message: err.message }));
      if (saveBtn) {
        saveBtn.textContent = this.tr("common.saveChanges");
        saveBtn.disabled = false;
      }
  ...
  async saveContentChanges() {
    if (!this.treeData) {
      alert(this.tr("content.noContentToSave"));
      return;
    }
    ...
    if (saveBtn) {
      saveBtn.disabled = true;
      saveBtn.textContent = this.tr("common.saving");
    }
    ...
    try {
      ...
      if (itemsToShow.length === 0 && itemsToHide.length === 0) {
        if (saveBtn) {
          saveBtn.textContent = this.tr("common.noChanges");
          setTimeout(() => {
            saveBtn.textContent = this.tr("common.saveChanges");
            saveBtn.disabled = false;
          }, 1500);
      ...
      console.log(
        `[SourceManager] Saving changes: ${itemsToShow.length} to show, ${itemsToHide.length} to hide`,
      );

      if (itemsToHide.length > 0) {
        console.log(
          "[SourceManager] Items to hide:",
          itemsToHide.map((i) => `${i.itemType}:${i.itemId}`),
        );
        // Check if any groups are being hidden
        const hiddenGroups = itemsToHide.filter(
          (i) => i.itemType === "group" || i.itemType.includes("category"),
        );
      ...
      const processBatches = async (items, apiFn, label) => {
        for (let i = 0; i < items.length; i += BATCH_SIZE) {
          ...
          console.log(
            `[SourceManager] ${label}: batch ${Math.floor(i / BATCH_SIZE) + 1}/${Math.ceil(items.length / BATCH_SIZE)} (${batch.length} items)`,
          );
          await apiFn(batch);
      ...
      try {
        if (window.app?.channelList) {
          ...
          if (
            window.app.channelList.currentSourceId &&
            String(window.app.channelList.currentSourceId) ===
              String(this.contentSourceSelect.value)
          ) {
            console.log(
              "[SourceManager] Reloading active source in ChannelList...",
            );
            await window.app.channelList.loadSource(
              window.app.channelList.currentSourceId,
            );
          } else {
      ...
      if (saveBtn) {
        saveBtn.textContent = this.tr("common.saved");
        setTimeout(() => {
          saveBtn.textContent = this.tr("common.saveChanges");
          saveBtn.disabled = false;
        }, 1500);
      ...
      alert(this.tr("content.failedSaveChanges", { message: err.message }));
      if (saveBtn) {
        saveBtn.textContent = this.tr("common.saveChanges");
        saveBtn.disabled = false;
      }
