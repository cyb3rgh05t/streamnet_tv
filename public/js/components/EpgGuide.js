...
class EpgGuide {
  ...
  async fetchEpgData(forceRefresh = false) {
    ...
    const sources = allSources.filter(
      (s) => (s.type === "epg" || s.type === "xtream") && s.enabled,
    );
    ...
    const fetchPromises = sources.map(async (source) => {
      try {
        return await API.request(
          "GET",
          `/proxy/epg/${source.id}${queryParams}`,
        );
      } catch (e) {
  ...
  getCurrentProgram(tvgId, channelName) {
    ...
    if (tvgId && this.channelMap && this.channelMap.has(tvgId)) {
      ...
      epgChannel = this.channels.find(
        (epg) => (tvgId && epg.id === tvgId) || epg.name === channelName,
      );
    }
  ...
  updateFilteredChannels() {
    ...
    if (!this.selectedGroup || this.selectedGroup === "All") {
      ...
      this.filteredChannels = this.allMatchedChannels.filter(
        (m) =>
          (m.sourceChannel.groupTitle || "Uncategorized") ===
          this.selectedGroup,
      );
    }
  }
  ...
  render() {
    ...
    const playableChannels = (channelList.channels || []).filter((ch) => {
      ...
      const isChannelHidden = channelList.isHidden(
        "channel",
        ch.sourceId,
        rawChannelId,
      );
    ...
    const allChannels = playableChannels.map((sourceChannel) => {
      ...
      const epgChannel = this.channels.find(
        (epg) =>
    ...
    this._nowIndicatorInterval = setInterval(
      () => this.updateNowIndicator(),
      60000,
    );
  }
  ...
  updateVisibleRows() {
    ...
    if (
      startIndex === this._lastVisibleStart &&
      endIndex === this._lastVisibleEnd
    ) {
      return;
    }
  ...
  createChannelRow(index) {
    ...
    const isFavorite = this.favorites.has(
      `${sourceChannel.sourceId}:${sourceChannel.id}`,
    );
    ...
    const logo = this.getProxiedImageUrl(
      sourceChannel.tvgLogo || (epgChannel && epgChannel.icon),
    );
  ...
  attachRowListeners(row) {
    ...
    row.querySelectorAll(".epg-program").forEach((prog) => {
      prog.addEventListener("click", () =>
        this.showProgramDetails(prog.dataset),
      );
    });
  ...
  async toggleFavorite(sourceId, channelId) {
    ...
    const rows = this.container.querySelectorAll(
      `.epg-channel-row[data-channel-id="${channelId}"][data-source-id="${sourceId}"]`,
    );
    rows.forEach((row) => {
  ...
  renderProgrammes(programmes, startTime, endTime) {
    ...
    for (const prog of programmes) {
      const progStart = Math.max(
        new Date(prog.start).getTime(),
        startTime.getTime(),
      );
      const progEnd = Math.min(
        new Date(prog.stop).getTime(),
        endTime.getTime(),
      );
  ...
  async playChannel(channelName) {
    ...
    if (window.app?.channelList) {
      const channel = window.app.channelList.channels.find(
        (c) => c.name === channelName || c.tvgName === channelName,
      );
      if (channel) {
