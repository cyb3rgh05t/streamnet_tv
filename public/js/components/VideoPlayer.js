...
class VideoPlayer {
  ...
  async loadSettingsFromServer() {
    try {
      ...
      console.warn(
        "[Player] Failed to load settings from server, using defaults:",
        err.message,
      );
      // Fall back to localStorage for backwards compatibility
      try {
        const saved = localStorage.getItem("streamnet_tv_player_settings");
        if (saved) {
          this.settings = {
        ...
        console.error(
          "[Player] Error loading localStorage settings:",
          localErr,
        );
      }
    }
  ...
  getHlsConfig() {
    ...
    return {
      ...
      // Ensure protected /api HLS endpoints include JWT header
      xhrSetup: (xhr, url) => {
        const token = localStorage.getItem("authToken");
        if (token && isProtectedApiUrl(url)) {
  ...
  initCustomControls() {
    ...
    if (this.controlsOverlay) {
      this.controlsOverlay.style.paddingBottom =
    ...
    const updatePlayUI = () => {
      ...
      const hasVideo =
        this.video.src && this.video.src !== "" && this.video.readyState > 0;
    ...
    const updateVolumeUI = () => {
      ...
      if (volumeSlider) {
        volumeSlider.value = this.video.muted
          ? 0
    ...
    window.addEventListener("channelChanged", (e) => {
      if (channelNameEl && e.detail) {
        channelNameEl.textContent =
          e.detail.name || e.detail.tvgName || "Live TV";
      }
      showOverlay();
    });
  ...
  updateCaptionsTracks() {
    ...
    this.captionsList.innerHTML =
      '<button class="captions-option" data-index="-1">Off</button>';
    ...
    if (this.video.textTracks && this.video.textTracks.length > 0) {
      ...
      for (let i = 0; i < this.video.textTracks.length; i++) {
        ...
        btn.textContent =
          track.label || `Track ${i + 1} (${track.language || "unknown"})`;
        btn.dataset.index = i;
  ...
  init() {
    ...
    const volume = this.settings.rememberVolume
      ? this.settings.lastVolume
    ...
    if (Hls.isSupported()) {
        ...
        if (data.fatal) {
          switch (data.type) {
            case Hls.ErrorTypes.NETWORK_ERROR:
              ...
              const now = Date.now();
              const timeSinceLastNetworkError =
                now - (this.lastNetworkErrorTime || 0);
              ...
              console.log(
                `Network error (attempt ${this.networkRetryCount}/3):`,
                data.details,
              );
          ...
          if (timeSinceLastRecovery > 2000) {
            console.log(
              `Non-fatal media error (${this.mediaErrorCount}x):`,
              data.details,
              "- attempting recovery",
            );
            ...
            if (this.mediaErrorCount >= 3) {
              console.log(
                "[HLS] Multiple errors detected, trying swapAudioCodec...",
              );
      ...
      this.hls.on(Hls.Events.FRAG_CHANGED, (event, data) => {
        ...
        console.log(
          `[HLS] FRAG_CHANGED: sn=${frag?.sn}, cc=${frag?.cc}, level=${frag?.level}`,
        );
        ...
        if (frag && frag.sn !== "initSegment") {
          ...
          if (frag.cc !== undefined && frag.cc !== this.lastDiscontinuity) {
            console.log(
              `[HLS] Discontinuity detected: CC ${this.lastDiscontinuity} -> ${frag.cc}`,
            );
  ...
  /**
   * Show persistent now playing panel
   */
  ...
  async startTranscodeSession(url, options = {}) {
    try {
      ...
      // Fallback to direct source to avoid protected /api/transcode URL without headers
      return url;
    }
  }
    ...
    try {
      ...
      if (this.settings.autoTranscode) {
        ...
        try {
          ...
          if (info.subtitles && info.subtitles.length > 0) {
            console.log(
              `[Player] Found ${info.subtitles.length} subtitle tracks`,
            );
          ...
          if (info.needsTranscode || this.settings.upscaleEnabled) {
            ...
            console.log(
              `[Player] Auto: Using HLS transcode session (${this.settings.upscaleEnabled ? "Upscaling" : "Incompatible audio/video"})`,
            );
          ...
          console.warn(
            "[Player] Probe failed, using normal playback:",
            err.message,
          );
      ...
      if (this.settings.forceVideoTranscode || this.settings.upscaleEnabled) {
        ...
        const statusMode = this.settings.upscaleEnabled
          ...
          : "transcoding";
        console.log(
          `[Player] ${statusText} enabled. Starting session (encode)...`,
        );
        this.updateTranscodeStatus(statusMode, statusText);
        const playlistUrl = await this.startTranscodeSession(streamUrl, {
          videoMode: "encode",
      ...
      const looksLikeHls =
        finalUrl.includes(".m3u8") || finalUrl.includes("m3u8");
      ...
      if (this.settings.forceRemux && (isRawTs || isExtensionless)) {
        console.log(
          "[Player] Force Remux enabled. Routing through FFmpeg remux...",
        );
        console.log(
          "[Player] Stream type:",
      ...
      if (isRawTs && !this.settings.forceRemux) {
        console.warn(
          "[Player] Raw MPEG-TS stream detected. Browsers cannot play .ts files directly.",
        );
        this.showError(
      ...
      if (looksLikeHls && Hls.isSupported()) {
        ...
        this.hls.on(Hls.Events.ERROR, (event, data) => {
          if (data.fatal) {
            ...
            const isLocalApi = (() => {
              try {
                const parsed = new URL(this.currentUrl, window.location.origin);
                return parsed.pathname.startsWith("/api/");
              } catch {
                return (
                  typeof this.currentUrl === "string" &&
                  this.currentUrl.startsWith("/api/")
                );
              }
            })();
            ...
            if (isCorsLikely && !this.isUsingProxy && !isLocalApi) {
              console.log(
                "CORS/Network error detected, retrying via proxy...",
                data.details,
              );
              this.isUsingProxy = true;
          ...
          if (!this.isUsingProxy) {
            ...
            this.video.play().catch((err) => {
              if (err.name !== "AbortError")
                console.error("Proxy play failed:", err);
            });
          }
        });
      } else {
    ...
    try {
      ...
      if (window.app && window.app.epgGuide && window.app.epgGuide.programmes) {
        ...
        if (currentProgram) {
          ...
          const epgChannel =
            epgGuide.channelMap?.get(channel.tvgId) ||
      ...
      if (channel.sourceType === "xtream" && channel.streamId) {
        const epgData = await API.proxy.xtream.shortEpg(
          channel.sourceId,
          channel.streamId,
        );
        if (
  ...
  updateNowPlaying(channel, epgData = null) {
    ...
    const tmdbTitleLogo = this.nowPlaying.querySelector(".program-tmdb-logo");
    ...
    const programDescription = this.nowPlaying.querySelector(
      ".program-description",
    );
    const channelLogo = this.nowPlaying.querySelector(".now-channel-logo");
    const upNextList = document.getElementById("up-next-list");

    channelName.textContent =
      channel.name || channel.tvgName || "Unknown Channel";
    ...
    if (channelLogo) {
      const rawLogoUrl =
        ...
        channel.icon ||
        "";
      const proxiedLogoUrl = window.app?.channelList?.getProxiedImageUrl
        ? window.app.channelList.getProxiedImageUrl(rawLogoUrl)
        : rawLogoUrl;

      if (proxiedLogoUrl) {
        channelLogo.src = proxiedLogoUrl;
        channelLogo.alt = `${channel.name || channel.tvgName || "Channel"} logo`;
        channelLogo.classList.remove("hidden");
      } else {
        channelLogo.removeAttribute("src");
        channelLogo.classList.add("hidden");
      }
    }

    if (epgData && epgData.current) {
      programTitle.textContent = epgData.current.title;
      const start = new Date(epgData.current.start).toLocaleTimeString([], {
        hour: "2-digit",
        minute: "2-digit",
      });
      const end = new Date(epgData.current.stop).toLocaleTimeString([], {
        hour: "2-digit",
        minute: "2-digit",
      });
      ...
      if (programDescription) {
        const rawPlot =
          epgData.current.description ||
          epgData.current.desc ||
          epgData.current.plot ||
          epgData.current.summary ||
          epgData.current.synopsis ||
          epgData.current.subTitle ||
          "";
        programDescription.textContent = this.decodeBase64(rawPlot);
      }
      this.updateTmdbCardArtwork(epgData.current.title);
    } else {
      ...
      if (tmdbTitleLogo) {
        tmdbTitleLogo.classList.add("hidden");
        tmdbTitleLogo.removeAttribute("src");
      }
      this.nowPlaying.classList.remove("has-tmdb");
      this.nowPlaying.style.setProperty("--tmdb-backdrop-url", "none");
    }

    // Update up next
    upNextList.innerHTML = "";
    if (epgData && epgData.upcoming) {
      const upcomingItems = this.mergeNearDuplicateProgrammes(
        epgData.upcoming,
        5,
      );
      const distinctUpcoming = [];
      const seenKeys = new Set();

      for (const prog of upcomingItems) {
        const titleKey = this.normalizeProgrammeTitle(prog.title);
        const summaryKey = this.normalizeProgrammeSummary(
          prog.description ||
            prog.desc ||
            prog.plot ||
            prog.summary ||
        ...
        const dedupeKey = `${titleKey}::${summaryKey}`;
        if (!titleKey || seenKeys.has(dedupeKey)) continue;
        seenKeys.add(dedupeKey);
        distinctUpcoming.push(prog);
        if (distinctUpcoming.length >= 3) break;
      }

      distinctUpcoming.forEach((prog) => {
        const li = document.createElement("li");
        li.className = "up-next-item";

        const time = new Date(prog.start).toLocaleTimeString([], {
          hour: "2-digit",
          minute: "2-digit",
        });
        ...
        const minutesUntil = Math.max(
          0,
          Math.round((new Date(prog.start).getTime() - Date.now()) / 60000),
        );

        const timeEl = document.createElement("span");
        timeEl.className = "up-next-time";
        timeEl.textContent = time;

        const titleEl = document.createElement("span");
        titleEl.className = "up-next-title";
        titleEl.textContent = prog.title || "Upcoming";

        const etaEl = document.createElement("span");
        etaEl.className = "up-next-eta";
        etaEl.textContent = `startet in ${minutesUntil} Minuten`;

        li.append(timeEl, titleEl, etaEl);
  ...
  applyTmdbCardArtwork(tmdbMeta) {
    const tmdbTitleLogo = this.nowPlaying.querySelector(".program-tmdb-logo");

    this.nowPlaying.classList.add("artwork-transition");
    if (this.tmdbTransitionTimer) {
      clearTimeout(this.tmdbTransitionTimer);
    }
    this.tmdbTransitionTimer = setTimeout(() => {
      this.nowPlaying.classList.remove("artwork-transition");
    }, 280);

    if (!tmdbMeta || !tmdbMeta.found) {
      this.nowPlaying.classList.remove("has-tmdb");
      this.nowPlaying.style.setProperty("--tmdb-backdrop-url", "none");
      if (tmdbTitleLogo) {
        tmdbTitleLogo.classList.add("hidden");
    ...
    if (tmdbMeta.backdropUrl) {
      const proxiedBackdrop = window.app?.channelList?.getProxiedImageUrl
        ? window.app.channelList.getProxiedImageUrl(tmdbMeta.backdropUrl)
        : tmdbMeta.backdropUrl;
      this.nowPlaying.style.setProperty(
        "--tmdb-backdrop-url",
        `url("${proxiedBackdrop}")`,
      );
      this.nowPlaying.classList.add("has-tmdb");
    } else {
      this.nowPlaying.classList.remove("has-tmdb");
      this.nowPlaying.style.setProperty("--tmdb-backdrop-url", "none");
    }

    if (tmdbTitleLogo && tmdbMeta.logoUrl) {
      const proxiedLogo = window.app?.channelList?.getProxiedImageUrl
        ? window.app.channelList.getProxiedImageUrl(tmdbMeta.logoUrl)
        : tmdbMeta.logoUrl;
      tmdbTitleLogo.classList.add("is-loading");
      tmdbTitleLogo.src = proxiedLogo;
      tmdbTitleLogo.alt = `${tmdbMeta.title || "Programme"} logo`;
      tmdbTitleLogo.onload = () => tmdbTitleLogo.classList.remove("is-loading");
      tmdbTitleLogo.onerror = () => tmdbTitleLogo.classList.add("hidden");
      tmdbTitleLogo.classList.remove("hidden");
    } else if (tmdbTitleLogo) {
      tmdbTitleLogo.classList.add("hidden");
      tmdbTitleLogo.removeAttribute("src");
    }
  }

  async updateTmdbCardArtwork(rawTitle) {
    const title = this.normalizeTmdbTitle(rawTitle);
    if (!title) {
      this.applyTmdbCardArtwork(null);
      return;
    }

    const cacheKey = title.toLowerCase();
    if (this.tmdbCache.has(cacheKey)) {
      this.applyTmdbCardArtwork(this.tmdbCache.get(cacheKey));
      return;
    }

    const lookupToken = ++this.tmdbLookupToken;
    try {
      const result = await API.proxy.tmdb.search(title);
      this.tmdbCache.set(cacheKey, result);
      if (lookupToken !== this.tmdbLookupToken) return;
      this.applyTmdbCardArtwork(result);
    } catch (err) {
