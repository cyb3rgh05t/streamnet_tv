...
class ChannelList {
  constructor() {
    ...
    this.selectedLiveGroup = null; // { groupName, sourceId|null }
    this.sources = [];
  ...
  loadCollapsedState() {
    try {
      const saved = localStorage.getItem("streamnet_tv_collapsed_groups");
      if (saved) {
  ...
  toggleAllGroups() {
    ...
    const allCollapsed = [...allHeaders].every((h) =>
  ...
  isLiveGroupsOnlyMode() {
    const livePage = document.getElementById("page-live");
    return Boolean(livePage && livePage.classList.contains("active"));
  }

  notifyLiveGroupChanged() {
    window.dispatchEvent(
      new CustomEvent("liveGroupChanged", {
        detail: this.selectedLiveGroup,
      }),
    );
  }
  ...
  getProgramInfo(channel) {
    try {
      ...
      const program = window.app.epgGuide.getCurrentProgram(
        channel.tvgId,
        channel.name,
      );
  ...
  render() {
    ...
    const isLiveGroupsOnly = this.isLiveGroupsOnlyMode();
    this.container.classList.toggle("live-groups-only", isLiveGroupsOnly);
    ...
    const favoritedChannels = this.channels.filter((ch) =>
      this.isFavorite(ch.sourceId, ch.id),
    );
    ...
    // 4. Build group order from provider category order (this.groups),
    //    Favorites always pinned to top.
    const allGroupKeys = Object.keys(groupedChannels);
    const groupKeySet = new Set(allGroupKeys);

    const providerOrderedGroups = [];
    const seenProviderGroups = new Set();
    this.groups.forEach((g) => {
      const name = g?.name;
      if (!name || seenProviderGroups.has(name)) return;
      if (!groupKeySet.has(name)) return;
      seenProviderGroups.add(name);
      providerOrderedGroups.push(name);
    });

    const remainingGroups = allGroupKeys.filter(
      (k) => k !== "Favorites" && !seenProviderGroups.has(k),
    );

    const allGroups = [
      ...(allGroupKeys.includes("Favorites") ? ["Favorites"] : []),
      ...providerOrderedGroups,
      ...remainingGroups,
    ];
    ...
    if (
      this.selectedLiveGroup &&
      !this.sortedGroups.includes(this.selectedLiveGroup.groupName)
    ) {
      this.selectedLiveGroup = null;
      this.notifyLiveGroupChanged();
    }
    ...
    this.sortedGroups.forEach((groupName) => {
      ...
      const visibleChannels = channels.filter((channel) => {
        ...
        const rawChannelId = channel.streamId || channel.id;
        const channelHidden = this.isHidden(
          "channel",
          channel.sourceId,
          rawChannelId,
        );
      ...
      const groupSourceIds = [
        ...new Set(visibleChannels.map((c) => String(c.sourceId))),
      ];
      const groupSourceId =
        groupSourceIds.length === 1 ? groupSourceIds[0] : "";
      const isSelectedLiveGroup =
        this.selectedLiveGroup &&
        this.selectedLiveGroup.groupName === groupName &&
        (!this.selectedLiveGroup.sourceId ||
          !groupSourceId ||
          this.selectedLiveGroup.sourceId === groupSourceId);
    ...
    if (this.sortedGroups.length === 0) {
      this.container.innerHTML = `
        <div class="empty-state">
          <p>${searchTerm ? this.tr("channels.noMatch") : this.tr("channels.noLoaded")}</p>
          <p class="hint">${searchTerm ? this.tr("channels.tryDifferent") : this.tr("channels.addSourceHint")}</p>
        </div>
      `;
      return;
    }
  ...
  renderNextBatch() {
    ...
    for (const groupName of groupsToRender) {
      ...
      if (
        !isFavoritesGroup &&
        !this.collapsedGroups.has(groupName) &&
      ...
      html += `
        <div class="channel-group">
          <div class="group-header ${this.collapsedGroups.has(groupName) ? "collapsed" : ""} ${isFavoritesGroup ? "favorites-group" : ""} ${isSelectedLiveGroup ? "active-live-group" : ""}" data-group="${groupName}" data-group-source-id="${groupSourceId}">
      ...
      for (const channel of visibleChannels) {
        ...
        const channelHidden =
          !isFavoritesGroup &&
          this.isHidden("channel", channel.sourceId, rawChannelId);
  ...
  attachGroupListeners(groupEl) {
    ...
    if (header) {
      header.addEventListener("click", () => {
        if (this.isLiveGroupsOnlyMode()) {
          this.selectLiveGroup(
            header.dataset.group,
            header.dataset.groupSourceId || null,
          );
          return;
        }
      ...
      header.addEventListener("contextmenu", (e) =>
        this.showContextMenu(e, "group", header.dataset),
      );
    }
    ...
    groupEl.querySelectorAll(".channel-item").forEach((item) => {
      ...
      item.addEventListener("contextmenu", (e) =>
        this.showContextMenu(e, "channel", item.dataset),
      );
      ...
      if (favBtn) {
        favBtn.addEventListener("click", (e) => {
          e.stopPropagation();
          this.toggleFavorite(
            parseInt(item.dataset.sourceId),
  ...
  renderGroupChannels(groupName, container) {
    ...
    for (const channel of visibleChannels) {
      ...
      const renderedChannel = this.renderedChannels.find(
        (rc) =>
          rc.id === channel.id &&
  ...
  async loadSources() {
    try {
      ...
      console.log(
        "[ChannelList] loadSources: Got",
        this.sources?.length || 0,
        "sources",
      );
      this.sourceSelect.innerHTML = `<option value="">${this.tr("common.allSources")}</option>`;

      const xtreamSources = this.sources.filter(
        (s) => s.type === "xtream" && s.enabled,
      );
      const m3uSources = this.sources.filter(
        (s) => s.type === "m3u" && s.enabled,
      );
  ...
  async loadAllChannels() {
    ...
    try {
      ...
      console.log(
        "[ChannelList] loadAllChannels: xtream=",
  ...
  async loadXtreamChannels(sourceId, append = false) {
    ...
    // Sort by provider channel number when available (bouquet-like order),
    // fallback to original API order.
    const sortedStreams = streams
      .map((stream, idx) => ({ stream, idx }))
      .sort((a, b) => {
        const aNum = Number(
          a.stream.stream_num ??
            a.stream.num ??
            a.stream.tvg_chno ??
            a.stream.channel_number,
        );
        const bNum = Number(
          b.stream.stream_num ??
            b.stream.num ??
            b.stream.tvg_chno ??
            b.stream.channel_number,
        );
    ...
    const channelList = sortedStreams.map((stream) => ({
  ...
  async loadHiddenItems() {
    try {
      ...
      this.hiddenItems = new Set(
        items.map((i) => `${i.item_type}:${i.source_id}:${i.item_id}`),
      );
    } catch (err) {
  ...
  async loadFavorites() {
    try {
      ...
      const channelFavs = allFavs.filter(
        (f) => !f.item_type || f.item_type === "channel",
      );
  ...
  async toggleFavorite(sourceId, channelId) {
    ...
    const btns = document.querySelectorAll(
      `.channel-item[data-channel-id="${channelId}"][data-source-id="${sourceId}"] .favorite-btn`,
    );
    ...
    try {
      ...
      const channel = this.channels.find(
        (c) => c.sourceId == sourceId && c.id == channelId,
      );
      if (channel) {
  ...
  updateFavoritesGroup(channel, isAdded) {
    ...
    const existingIdx = favArray.findIndex(
      (c) => c.id === channel.id && c.sourceId === channel.sourceId,
    );
    ...
    // 2. Update DOM
    const groupHeader = this.listContainer.querySelector(
      '.group-header[data-group="Favorites"]',
    );
    ...
    if (isAdded) {
      ...
      const existingEl = groupChannels.querySelector(
        `.channel-item[data-channel-id="${channel.id}"][data-source-id="${channel.sourceId}"]`,
      );
      if (!existingEl) {
  ...
  createChannelElement(channel) {
    ...
    div.addEventListener("contextmenu", (e) =>
      this.showContextMenu(e, "channel", div.dataset),
    );
  ...
  async selectChannel(dataset) {
    ...
    let activeItem;
    activeItem = this.container.querySelector(
      `[data-render-id="${this.currentRenderId}"]`,
    );
    ...
    if (!activeItem && this.renderedChannels.length > 0) {
      ...
      while (
        !activeItem &&
        this.currentBatch * this.batchSize < this.sortedGroups.length &&
        safety < 20
      ) {
        this.renderNextBatch();
    ...
    if (activeItem) {
      ...
      const groupHeader = activeItem
        .closest(".channel-group")
  ...
  showContextMenu(e, type, data) {
    ...
    this.contextMenu.dataset.itemId =
      type === "group" ? data.group : data.channelId;
  ...
  showEpgInfo(sourceId, channelId, streamId) {
    ...
    let programsHtml =
      '<p class="no-programs">No EPG data available for this channel.</p>';

    if (window.app?.epgGuide) {
      const tvgKey = channel.tvgId || channel.name;
      const currentProgram = window.app.epgGuide.getCurrentProgram(
        channel.tvgId,
        channel.name,
      );
      ...
      if (currentProgram || programs.length > 0) {
        ...
        if (currentProgram) {
          const startTime = new Date(currentProgram.start).toLocaleTimeString(
            [],
            { hour: "2-digit", minute: "2-digit" },
          );
          const endTime = new Date(currentProgram.end).toLocaleTimeString([], {
            hour: "2-digit",
            minute: "2-digit",
          });
          programsHtml += `
        ...
        upcoming.forEach((prog) => {
          const startTime = new Date(prog.start).toLocaleTimeString([], {
            hour: "2-digit",
            minute: "2-digit",
          });
          const endTime = new Date(prog.end).toLocaleTimeString([], {
            hour: "2-digit",
            minute: "2-digit",
          });
          programsHtml += `
  ...
  selectNextChannel() {
    if (
      !this.currentChannel ||
      !this.renderedChannels ||
      this.renderedChannels.length === 0
    )
      return;
    ...
    if (this.currentRenderId) {
      currentIndex = this.renderedChannels.findIndex(
        (c) => c._renderId === this.currentRenderId,
      );
    }
    ...
    if (currentIndex === -1) {
      ...
      if (this.currentRenderGroup) {
        currentIndex = this.renderedChannels.findIndex(
          (c) =>
            c.id === this.currentChannel.id &&
            c.sourceId === this.currentChannel.sourceId &&
            c._renderGroup === this.currentRenderGroup,
        );
      }
  ...
  selectPrevChannel() {
    ...
    const prevIndex =
      (currentIndex - 1 + this.renderedChannels.length) %
      this.renderedChannels.length;
