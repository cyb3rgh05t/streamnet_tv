...
class SettingsPage {
  constructor(app) {
    this.app = app;
    this.tabs = document.querySelectorAll("#page-settings .settings-nav .tab");
    this.tabContents = document.querySelectorAll(
      "#page-settings .settings-main > .tab-content",
    );
  ...
  initPlayerSettings() {
    ...
    const overlayDurationInput = document.getElementById(
      "setting-overlay-duration",
    );
    const defaultVolumeSlider = document.getElementById(
      "setting-default-volume",
    );
    const volumeValueDisplay = document.getElementById("volume-value");
    const rememberVolumeToggle = document.getElementById(
      "setting-remember-volume",
    );
    ...
    if (epgRefreshSelect && this.app.player?.settings) {
      ...
      epgRefreshSelect.value =
        this.app.player.settings.epgRefreshInterval || "24";
  ...
  async initTranscodingSettings() {
    ...
    const maxResolutionSelect = document.getElementById(
    ...
    const autoTranscodeToggle = document.getElementById(
      "setting-auto-transcode-tc",
    );
    const forceTranscodeToggle = document.getElementById(
      "setting-force-transcode-tc",
    );
    const forceVideoTranscodeToggle = document.getElementById(
      "setting-force-video-transcode-tc",
    );
    const forceRemuxToggle = document.getElementById("setting-force-remux-tc");
    const streamFormatSelect = document.getElementById(
    ...
    const userAgentCustomInput = document.getElementById(
      "setting-user-agent-custom-tc",
    );
    const customUaContainer = document.getElementById(
      "custom-user-agent-container-tc",
    );
    ...
    try {
      ...
      console.warn(
        "[Settings] Failed to load settings from API, using player defaults:",
        err,
      );
    ...
    if (maxResolutionSelect)
      maxResolutionSelect.value = s.maxResolution || "1080p";
    ...
    if (autoTranscodeToggle)
      autoTranscodeToggle.checked = s.autoTranscode !== false;
    if (forceTranscodeToggle)
      forceTranscodeToggle.checked = s.forceTranscode === true;
    if (forceVideoTranscodeToggle)
      forceVideoTranscodeToggle.checked = s.forceVideoTranscode === true;
    ...
    if (userAgentCustomInput)
      userAgentCustomInput.value = s.userAgentCustom || "";
    if (customUaContainer) {
      customUaContainer.style.display =
        userAgentSelect?.value === "custom" ? "flex" : "none";
    }
    ...
    // Upscaling Settings
    const upscaleEnabledToggle = document.getElementById(
      "setting-upscale-enabled",
    );
    const upscaleMethodSelect = document.getElementById(
      "setting-upscale-method",
    );
    const upscaleTargetSelect = document.getElementById(
      "setting-upscale-target",
    );
    const upscaleMethodContainer = document.getElementById(
      "upscale-method-container",
    );
    const upscaleTargetContainer = document.getElementById(
      "upscale-target-container",
    );

    // Helper to toggle upscale options visibility
    const toggleUpscaleOptions = (enabled) => {
      if (upscaleMethodContainer)
        upscaleMethodContainer.style.display = enabled ? "flex" : "none";
      if (upscaleTargetContainer)
        upscaleTargetContainer.style.display = enabled ? "flex" : "none";
    };
    ...
    if (upscaleMethodSelect)
      upscaleMethodSelect.value = s.upscaleMethod || "hardware";
    if (upscaleTargetSelect)
      upscaleTargetSelect.value = s.upscaleTarget || "1080p";
    ...
    forceVideoTranscodeToggle?.addEventListener("change", () => {
      this.app.player.settings.forceVideoTranscode =
        forceVideoTranscodeToggle.checked;
      this.app.player.saveSettings();
    });
  ...
  async loadHardwareInfo() {
    ...
    try {
      ...
      if (detected.length > 0) {
        ...
        html += `<p class="hint" style="margin-top: var(--space-sm);">${this.tr("settings.recommendedEncoder")}: <strong>${hwInfo.recommended}</strong></p>`;
      } else {
        html = `<p class="hint">${this.tr("settings.noGpuDetected")}</p>`;
      }

      container.innerHTML = html;
    } catch (err) {
      console.error("Error loading hardware info:", err);
      container.innerHTML = `<p class="hint error">${this.tr("settings.failedLoadHw")}</p>`;
    }
  }
    ...
    if (addUserForm) {
      addUserForm.addEventListener("submit", async (e) => {
        ...
        try {
          ...
          alert(this.tr("users.createdSuccess"));
          addUserForm.reset();
          this.loadUsers();
        } catch (err) {
          alert(this.tr("users.errorCreate", { message: err.message }));
        }
      });
    }
  }
  ...
  async loadUsers() {
    ...
    try {
      ...
      if (users.length === 0) {
        userList.innerHTML = `<tr><td colspan="5" class="hint">${this.tr("users.noneFound")}</td></tr>`;
        return;
      }
      ...
      userList.innerHTML = users
        .map((user) => {
          ...
          const roleBadge =
            user.role === "admin"
              ? `<span class="user-badge user-badge-admin">${this.tr("users.roleAdmin")}</span>`
              : `<span class="user-badge user-badge-viewer">${this.tr("users.roleViewer")}</span>`;
          ...
          return `
                <tr>
                    ...
                    <td>
                      <button class="btn btn-sm btn-secondary" onclick="window.app.pages.settings.openEditUserModal(${user.id})">${this.tr("users.edit")}</button>
                      <button class="btn btn-sm btn-error" onclick="window.app.pages.settings.deleteUser(${user.id}, '${user.username}')">${this.tr("users.delete")}</button>
                    </td>
                </tr>
            `;
        })
      ...
      userList.innerHTML = `<tr><td colspan="5" class="hint">${this.tr("users.errorLoad")}</td></tr>`;
    }
  }

  openEditUserModal(userId) {
    console.log(
      "openEditUserModal called with ID:",
      userId,
      "Type:",
      typeof userId,
    );
    ...
    if (!modal) {
      console.error(
        "CRITICAL: Modal element #edit-user-modal not found in DOM!",
      );
      alert(this.tr("users.errorModalMissing"));
      return;
    }
    ...
    try {
      ...
      if (isSSO) {
        if (editPassword) {
          ...
          editPassword.placeholder = this.tr("users.passwordManagedBySso");
        }
        if (passwordHint)
          passwordHint.textContent = this.tr("users.passwordNoChangeSso");
        ...
        if (editPassword) {
          ...
          editPassword.placeholder = this.tr("users.passwordLeaveBlank");
        }
        if (passwordHint)
          passwordHint.textContent = this.tr("users.passwordHint");
      ...
      alert(this.tr("users.errorOpenModal", { message: err.message }));
    }
  }
  ...
  setupModalHandlers(modal) {
    ...
    saveBtn.onclick = async () => {
      ...
      try {
        ...
        alert(this.tr("users.errorUpdate", { message: err.message }));
      }
    };
  ...
  async deleteUser(userId, username) {
    if (!confirm(this.tr("users.confirmDelete", { username }))) {
      return;
    }
    ...
    try {
      ...
      alert(this.tr("users.errorDelete", { message: err.message }));
    }
  }
  ...
  async show() {
    ...
    // Sources tab: visible for all users
    const sourcesTab = document.querySelector(
      '#page-settings .settings-nav .tab[data-tab="sources"]',
    );
    ...
    if (this.app.player?.settings) {
      ...
      if (volumeValueDisplay)
        volumeValueDisplay.textContent = s.defaultVolume + "%";
      if (rememberVolumeToggle) rememberVolumeToggle.checked = s.rememberVolume;
      if (autoPlayNextToggle)
        autoPlayNextToggle.checked = s.autoPlayNextEpisode;
      if (forceProxyToggle) forceProxyToggle.checked = s.forceProxy || false;
      if (forceTranscodeToggle)
        forceTranscodeToggle.checked = s.forceTranscode || false;
      if (forceRemuxToggle) forceRemuxToggle.checked = s.forceRemux || false;
      if (autoTranscodeToggle)
        autoTranscodeToggle.checked = s.autoTranscode || false;
      if (epgRefreshSelect)
        epgRefreshSelect.value = s.epgRefreshInterval || "24";
      if (streamFormatSelect)
        streamFormatSelect.value = s.streamFormat || "m3u8";

      // User-Agent settings
      const userAgentSelect = document.getElementById("setting-user-agent");
      const userAgentCustomInput = document.getElementById(
        "setting-user-agent-custom",
      );
      const customUaContainer = document.getElementById(
        "custom-user-agent-container",
      );
      if (userAgentSelect) {
  ...
  async updateEpgLastRefreshed() {
    ...
    try {
      ...
      if (data.lastSyncTime) {
        ...
        let text;
        if (diffMins < 1) {
          text = this.tr("time.justNow");
        } else if (diffMins < 60) {
          text = this.tr("time.minutesAgo", { count: diffMins });
        } else if (diffHours < 24) {
          text = this.tr("time.hoursAgo", { count: diffHours });
        } else {
        ...
        display.textContent = this.tr("epg.never");
        display.title = this.tr("settings.syncNotRunYet");
      }
    } catch (err) {
      console.error("Error fetching sync status:", err);
      display.textContent = this.tr("common.unknown");
      display.title = this.tr("settings.syncStatusUnavailable");
    }
  }
