...
1.  Clone the repository:
    ...
    ```bash
    git clone https://github.com/technomancer702/streamnet_tv.git streamnet_tv
    cd streamnet_tv
    ```
...
You can run StreamNet TV easily using Docker.
...
1.  Create a `docker-compose.yml` file (or copy the one from this repo):
    ...
    services:
      streamnet-tv:
        build: https://github.com/technomancer702/streamnet_tv.git#main
        container_name: streamnet-tv
        ports:
...
### Xtream Multi-User Login Mode (Server/SaaS)

If you want to host one central instance and let users sign in with their own Xtream credentials (without local installation), enable Xtream auth mode:

```env
XTREAM_AUTH_ENABLED=true
XTREAM_PORTAL_URL=https://xui.streamnet.live
```

How it works:

- Login uses the provided Xtream username/password against `XTREAM_PORTAL_URL`.
- A local app user is auto-provisioned per Xtream account.
- A dedicated Xtream source is auto-created/updated per user.
- Sources/channels/proxy/settings are isolated per authenticated user.
...
```env
OIDC_ISSUER_URL=https://your-idp.com/application/o/streamnet-tv/
...
StreamNet TV is a web-based application. By default, **video decoding is handled by your browser**. However, the built-in **smart transcoding system** automatically converts incompatible media (e.g., HEVC video, Dolby audio) into browser-friendly formats using FFmpeg.
...
StreamNet TV is optimized for **HLS (HTTP Live Streaming)**.
...
If you're running StreamNet TV behind a reverse proxy (Nginx, Caddy, Traefik) with HTTPS:
...
```nginx
location / {
  proxy_pass http://streamnet:3000;
...
1.  **Force Backend Proxy:** Enable this in **Settings → Transcoding → Network**. This routes middleware streams through StreamNet TV, bypassing CORS restrictions.
...
If you're using TVHeadend as your source, you may need to configure a few settings for streams to play correctly in StreamNet TV:

**Option 1: Enable Force Backend Proxy (Easiest)**

- In StreamNet TV, go to **Settings → Transcoding → Network**
...
- Add your StreamNet TV URL to **"CORS origin"** (e.g., `http://192.168.1.100:3000`)
...
If you are using `acestream-docker-home` or similar tools, it is **recommended** to use the HLS output format to reduce server load, though StreamNet TV can remux raw streams if needed.
