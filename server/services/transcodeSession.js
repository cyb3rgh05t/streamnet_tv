...
class TranscodeSession extends EventEmitter {
  constructor(url, options = {}) {
    super();
    const toPositiveNumberOr = (value, fallback) => {
  ...
  async start() {
    ...
    console.log(
      `[TranscodeSession ${this.id}] Starting session for: ${this.url}`,
    );
    ...
    console.log(
      `[TranscodeSession ${this.id}] Command: ${this.options.ffmpegPath} ${args.join(" ")}`,
    );
    ...
    try {
      ...
      this.process.on("exit", (code) => {
        if (code === 0 || code === null) {
          console.log(
            `[TranscodeSession ${this.id}] FFmpeg completed successfully`,
          );
          this.status = "stopped";
        } else if (code !== 255) {
          // 255 is often from SIGKILL
          console.error(
            `[TranscodeSession ${this.id}] FFmpeg exited with code ${code}`,
          );
          this.status = "error";
  ...
  buildFFmpegArgs() {
    ...
    if (encoder === "auto") {
      ...
      console.log(
        `[TranscodeSession ${this.id}] Auto encoder resolved to: ${encoder}`,
      );
    }
    ...
    if (videoMode === "copy") {
      ...
      if (
        this.options.videoCodec === "hevc" ||
        this.options.videoCodec === "h265"
      ) {
        args.push("-bsf:v", "hevc_mp4toannexb");
      } else if (
        this.options.videoCodec === "h264" ||
        this.options.videoCodec === "avc"
      ) {
    ...
    const AUDIO_MIX_FILTERS = {
      ...
      night:
        "pan=stereo|FL=0.5*FL+1.2*FC+0.3*BL+0.1*LFE|FR=0.5*FR+1.2*FC+0.3*BR+0.1*LFE",
      // Cinematic: Wide soundstage, immersive (original "dialogue boost" mix)
      cinematic:
        "pan=stereo|FL=FC+0.80*FL+0.60*BL+0.5*LFE|FR=FC+0.80*FR+0.60*BR+0.5*LFE",
    };
    ...
    if (audioMixPreset === "passthrough") {
      ...
      console.log(
        `[TranscodeSession ${this.id}] Audio: Auto (Smart Copy) - Source is Stereo AAC`,
      );
      args.push("-c:a", "copy");
    } else if (audioMixPreset === "auto") {
      // Safe default for multichannel sources: let FFmpeg do the stereo fold-down.
      // This avoids clipping/robotic artifacts that can happen with aggressive custom pan matrices.
      console.log(
        `[TranscodeSession ${this.id}] Audio: Auto safe downmix (${audioCodec} ${audioChannels}ch -> Stereo AAC)`,
      );
      args.push(
        "-c:a",
        "aac",
      ...
      console.log(
        `[TranscodeSession ${this.id}] Audio: ${mixPreset.toUpperCase()} mix (${audioCodec} ${audioChannels}ch -> Stereo AAC)`,
      );
      args.push(
        "-c:a",
        "aac",
        ...
        "192k",
        "-af",
        `${panFilter},volume=0.72,aresample=async=1:first_pts=0:min_hard_comp=0.100000`,
      );
    }
  ...
  getTargetHeight() {
    ...
    if (this.options.upscaleEnabled) {
      ...
      console.log(
        `[TranscodeSession ${this.id}] Upscale target height: ${target}p`,
      );
      return target;
    }
  ...
  buildScaleFilter(encoder, height) {
    ...
    if (useUpscale) {
      console.log(
        `[TranscodeSession ${this.id}] Upscaling: ${upscaleMethod} method to ${height}p`,
      );
    }
  ...
  addNvencEncoderArgs(args, height, qp) {
    ...
    args.push(
      ...
      "h264_nvenc",
      "-preset",
      "p4", // Balanced preset (p1=fastest, p7=best)
      "-rc",
  ...
  addVaapiEncoderArgs(args, height, qp) {
    ...
    args.push(
      ...
      "h264_vaapi",
      "-profile:v",
      "main", // Use main profile for compatibility
      "-global_quality",
      String(qp),
      "-bf",
      "3",
  ...
  addSoftwareEncoderArgs(args, height, crf) {
    ...
    args.push(
      ...
      "libx264",
      "-preset",
      "veryfast", // Fast for real-time
      "-crf",
      String(crf),
      "-profile:v",
      "high",
      "-pix_fmt",
      "yuv420p", // Force 8-bit output for compatibility (fixes 10-bit input errors)
    );
  }
  ...
  static async restore(sessionDir) {
    ...
    try {
      ...
      console.error(
        `Failed to restore session from ${sessionDir}:`,
        err.message,
      );
      return null;
    }
  ...
  async cleanup() {
    ...
    try {
      ...
      console.error(
        `[TranscodeSession ${this.id}] Failed to cleanup:`,
        err.message,
      );
    }
  }
...
async function recoverSessions() {
  try {
    ...
    if (err.code !== "ENOENT") {
      console.error(
        "[TranscodeSession] Error recovering sessions:",
        err.message,
      );
    }
  }
