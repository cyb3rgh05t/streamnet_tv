...
class SyncService {
  ...
  async startSyncTimer() {
    ...
    if (this._currentInterval === intervalHours && this._syncTimer) {
      console.log(
        `[Sync] Timer already running for ${intervalHours} hours, not restarting`,
      );
      return;
    }
  ...
  async syncSource(sourceId) {
    ...
    try {
      ...
      console.log(
        `[Sync] Starting sync for source ${source.name} (ID: ${sourceId})`,
      );
  ...
  async syncXtream(source) {
    ...
    try {
      ...
      console.warn(
        "[Sync] XMLTV fetch failed, skipping EPG sync for now:",
        e.message,
      );
    }
  }
  ...
  async saveCategories(sourceId, type, categories) {
    ...
    console.log(
      `[Sync] Saving ${categories.length} ${type} categories for source ${sourceId}...`,
    );
    const db = getDb();
    const stmt = db.prepare(`
    ...
    const insertBatch = db.transaction((batch, baseIndex) => {
      for (let i = 0; i < batch.length; i++) {
        const cat = batch[i];
        const catId = cat.category_id; // standard xtream field
        const name = cat.category_name;
        const id = `${sourceId}:${catId}`;
        const sortOrder = Number.isFinite(Number(cat.sort_order))
    ...
    for (let i = 0; i < categories.length; i += BATCH_SIZE) {
      insertBatch(categories.slice(i, i + BATCH_SIZE), i);
  ...
  async purgeStaleItems(sourceId, type, syncedIds) {
    ...
    const insertTemp = db.prepare(
      "INSERT OR IGNORE INTO synced_ids (id) VALUES (?)",
    );
  ...
  async syncEpgFromUrl(sourceId, url) {
    ...
    const logMemory = () => {
      ...
      console.log(
        `[Sync] Memory: ${Math.round(used.heapUsed / 1024 / 1024)}MB heap`,
      );
    };
    ...
    console.log(
      `[Sync] EPG Parsed: ${allChannels.length} channels, ${totalProgrammes} programmes`,
    );
    logMemory();
  ...
  async syncM3u(source) {
    ...
    for await (const batch of m3uParser.fetchAndParseStreaming(source.url)) {
      ...
      if (playlistItems.length > 0) {
        const batchIds = await this.saveStreams(
          source.id,
          "live",
          playlistItems,
          { skipPurge: true },
    ...
    console.log(
      `[Sync] M3U Parsed: ${totalChannels} channels, ${allGroups.size} groups`,
    );
    logMemory();
