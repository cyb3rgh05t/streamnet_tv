...
function findFFmpeg() {
  ...
  try {
    ...
    console.warn(
      "FFmpeg not available - transcoding/remuxing will be disabled.",
    );
    console.warn(
      "Install FFmpeg via your package manager or npm install ffmpeg-static",
    );
    return null;
  }
}
...
function findFFprobe() {
  ...
  console.warn(
    "FFprobe not available - auto transcode will fallback to always transcode",
  );
  return null;
}
...
try {
  ...
  console.warn(
    "No services directory found or failed to read services:",
    e.message,
  );
}
      ...
      for (const file of pluginFiles) {
        ...
        try {
          ...
          if (typeof plugin === "function") {
            ...
            console.warn(
              `⚠ Plugin ${file} does not export a function or object with init(), skipping.`,
            );
          }
        } catch (err) {
...
// Require auth for all API routes except /api/auth, /api/version, and /api/proxy/image
// (image proxy must be public because browsers load <img src> without Authorization headers)
app.use("/api", (req, res, next) => {
  if (req.path === "/version") return next();
  if (req.path.startsWith("/proxy/image")) return next();
...
app.listen(PORT, async () => {
  console.log(`StreamNet TV server running on http://localhost:${PORT}`);
