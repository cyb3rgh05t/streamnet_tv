...
const JWT_SECRET =
  process.env.JWT_SECRET || "streamnet-tv-secret-key-change-in-production";
    ...
    new OpenIDConnectStrategy(
      {
        ...
        userInfoURL:
          ...
          `${process.env.OIDC_ISSUER_URL}/protocol/openid-connect/userinfo`,
      ...
      async (...args) => {
        ...
        if (!sub) {
          return done(
            new Error("Could not identify OIDC Subject (sub) from arguments"),
          );
        }
        ...
        try {
          ...
          if (!user) {
            const username =
              profile.username ||
              profile.displayName ||
              (email ? email.split("@")[0] : `user_${sub.substring(0, 8)}`);
...
function requireRole(role) {
  return (req, res, next) => {
    if (!req.user || req.user.role !== role) {
      return res
        .status(403)
        .json({ error: `Forbidden - ${role} access required` });
    }
    next();
  };
}
