...
router.post("/", (req, res) => {
  try {
    ...
    const normalizedSourceId = Number.isFinite(Number(sourceId))
      ? Number(sourceId)
      : Number.isFinite(Number(data?.sourceId))
    ...
    // Persist session events in an append-only table so analytics keep full history.
    if (eventType === "session_start") {
      ...
      const lastEventTs = Number(lastEventRow?.created_at) || 0;
      if (!lastEventTs || timestamp - lastEventTs >= cooldownMs) {
        db.prepare(
          `
                    INSERT INTO watch_events (user_id, source_id, item_type, item_id, event_type, created_at, meta)
                    VALUES (?, ?, ?, ?, ?, ?, ?)
                `,
        ).run(
          userId,
