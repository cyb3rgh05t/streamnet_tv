...
router.post("/session", async (req, res) => {
  ...
  try {
    const session = await transcodeSession.createSession(url, {
      ...
      probeSize:
        Number.isFinite(parsedProbeSize) && parsedProbeSize > 0
          ? parsedProbeSize
          : undefined,
      analyzeDuration:
        Number.isFinite(parsedAnalyzeDuration) && parsedAnalyzeDuration > 0
          ? parsedAnalyzeDuration
          : undefined,
      hlsTime:
        Number.isFinite(parsedHlsTime) && parsedHlsTime >= 1
          ? parsedHlsTime
          : undefined,
    });
    ...
    if (!ready) {
      ...
      return res
        .status(500)
        .json({ error: "Transcoding failed to start", reason });
    }
...
router.get("/", async (req, res) => {
  ...
  const args = [
    ...
    "0:v:0",
    "-map",
    "0:a:0?", // ? makes audio optional if not present
  ...
  try {
    ...
    return res
      .status(500)
      .json({ error: "FFmpeg spawn failed", details: spawnErr.message });
  }
