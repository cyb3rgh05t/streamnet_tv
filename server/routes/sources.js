...
router.get("/:id", async (req, res) => {
  try {
    const source = await sources.getById(req.params.id, req.user.id);
    if (!source) {
...
router.delete("/:id", async (req, res) => {
  try {
    ...
    const deleteCategories = db.prepare(
      "DELETE FROM categories WHERE source_id = ?",
    );
    const deleteItems = db.prepare(
      "DELETE FROM playlist_items WHERE source_id = ?",
    );
    const deleteEpg = db.prepare(
      "DELETE FROM epg_programs WHERE source_id = ?",
    );
    const deleteSyncStatus = db.prepare(
      "DELETE FROM sync_status WHERE source_id = ?",
    );
    ...
    console.log(
      `[Source] Cascade delete for source ${sourceId}: ${catResult.changes} categories, ${itemResult.changes} items, ${epgResult.changes} EPG programs`,
    );
...
router.post("/:id/sync", async (req, res) => {
  try {
    ...
    const source = await sources.getById(id, req.user.id);
...
router.post("/:id/test", async (req, res) => {
  try {
    ...
    if (source.type === "xtream") {
      const result = await xtreamApi.authenticate(
        source.url,
        source.username,
        source.password,
      );
      ...
      res.json({
        success: isValid,
        message: isValid ? "Valid M3U playlist" : "Invalid M3U format",
      });
      ...
      res.json({
        success: isValid,
        message: isValid ? "Valid EPG XML" : "Invalid EPG format",
      });
    }
  } catch (err) {
