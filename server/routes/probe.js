...
function analyzeProbeResult(probeResult, url) {
  ...
  const rawDuration = Number(videoStream?.duration || format?.duration || 0);
  const duration =
  ...
  const isRawTs =
    (container.includes("mpegts") || url.endsWith(".ts")) &&
    !url.includes(".m3u8");
  ...
  const subtitles = streams
    .filter(
      (s) =>
        ...
        s.codec_name !== "timed_id3" &&
        s.codec_name !== "bin_data",
    )
    .map((s) => ({
      index: s.index,
...
router.get("/", async (req, res) => {
  ...
  console.log(
    `[Probe] Probing: ${url.substring(0, 80)}... ${ua ? `(UA: ${ua})` : ""}`,
  );
  ...
  try {
    ...
    console.log(
      `[Probe] Result: video=${analysis.video}, audio=${analysis.audio}, ` +
