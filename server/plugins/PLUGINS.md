# StreamNet TV Plugin System

This directory allows you to extend the functionality of **StreamNet TV** without modifying the core source code. The server automatically detects and loads any `.js` file placed in this folder at startup.
...
module.exports = async function (app, services) {
  ...
  const epgData = await services.epgParser.fetchAndParse(
...
2. Restart the StreamNet TV server
